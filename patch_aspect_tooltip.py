...
# 1. Add showAspectTooltip function
aspect_tooltip_func = """
...
content = content.replace("function showItemTooltip(", aspect_tooltip_func + "\nfunction showItemTooltip(")

# 2. Add event listeners in renderAspectTab
render_aspect_target = """      row.appendChild(icon);
...
render_aspect_replace = """      row.appendChild(icon);
      row.appendChild(name);
...
content = content.replace(render_aspect_target, render_aspect_replace)

# 3. Add to renderStashTab as well (for equipping aspects? Wait, the user asked for "mousing over an aspect in the item modal", this usually means the Aspect tab)
with open("app.js", "w", encoding="utf-8") as f:
    f.write(content)
print("Added Aspect Tooltip logic")
