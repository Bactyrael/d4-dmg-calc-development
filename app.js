...
function renderEquipment(className, savedEquipment = {}) {
  ...
  function cleanStatName(name) {
    ...
    let cleaned = name.replace(/^\+?\[[\d\.,]+\s*-\s*([\d\.,]+)\](%?)\s*/, (match, p1, p2) => p2 ? '% ' : '');
...
function calculateSkillAdditiveBucket(skill, isHit) {
    ...
    if (tags.some(t => t.includes('summon'))) {
        ...
        if (skill.name && skill.name.toLowerCase().includes('mage')) { addStat('Skeletal Mage Damage'); addStat('Skeleton Mage Damage'); }
...
function renderCalcSkills() {
                            ...
                            let b = getSkillDamageBreakdown(modSkill, rank);
                              ...
                              if (modSkill.baseDamageScalar) {
                                  ...
                                  html += `<details style="margin-bottom: 4px;">
                                    <summary style="cursor: pointer; display: flex; flex-direction: column; gap: 4px; outline: none;">
                                      <div style="display: flex; align-items: center; gap: 5px;">
                                        <span style="color: #555;">├</span> ${baseLabel} (${pctDisplay}): <span style="color: #fff; font-weight: bold;">${b.minStr} - ${b.maxStr}</span>
                                      </div>
                                      ${b.isHit ? `<div style="display: flex; align-items: center; gap: 5px; margin-left: 17px;">
                                        <span style="color: #555;">└</span> Critical Strike Damage: <span style="color: #fff; font-weight: bold;">${b.critStrMin} - ${b.critStrMax}</span>
                                      </div>` : ''}
                                    </summary>
                                    ...
                                    ${!b.isHit ? '' : `<details style="margin-left: 20px; font-size: 0.9em; margin-bottom: 6px;">
                                      <summary style="cursor: pointer; display: flex; align-items: center; gap: 5px; outline: none; color: #f9d85c;">
                                        <span style="color: #555;">└</span> Critical Strike Chance: <span style="font-weight: bold;">${b.critChanceUncapped > 100 ? Number(b.critChanceUncapped.toFixed(1)) + ' / 100' : Number(b.critChance.toFixed(1))}%</span>
                                      </summary>
                              ...
                              if (baseSkill.name === 'Skeleton Warrior' && typeof currentBuild !== 'undefined' && currentBuild && currentBuild.bookOfTheDead && currentBuild.bookOfTheDead.warriors) {
                                  if (currentBuild.bookOfTheDead.warriors.spec === 'Defender' && currentBuild.bookOfTheDead.warriors.node === '1') {
                                      ...
                                      let thornsSkillObj = {
                                          ...
                                          tags: ['Minion', 'Summoning', 'Thorns', 'Physical', 'Damage', 'Blood', 'Skill_Blood', 'Search_Blood', 'Corpse', 'Skill_Corpse'],
    ...
    let playerThornsVal = (window.D4_COMPILED_STATS && window.D4_COMPILED_STATS['Thorns']) ? window.D4_COMPILED_STATS['Thorns'].final : 0;
...
function getSkillDamageBreakdown(skillObj, displayRank, isHit) {
    ...
    return {
        ...
        critMultiMult,
        critAdditiveMult,
        additiveComponents: addData.components.concat({ name: 'Additive Critical Bonus', value: additiveCritBonus }),
        multiplicativeComponents: multiData.components.concat(critMultiplicativeComponents),
