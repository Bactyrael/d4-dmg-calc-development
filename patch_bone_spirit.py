...
orderings = {
    "Bone Spirit": ["Poltergeists", "Unfinished Business", "Astral Projection", "Charges", "Core Skill", "Damage Bonus", "Maximum Essence"]
}

for base_skill, order in orderings.items():
    # Use re.escape but replace literal spaces with \s+ to handle "bone  spirit" vs "bone spirit"
    base_skill_pattern = re.escape(base_skill).replace(r'\ ', r'\s+')
    pattern = r"([\"']name[\"']:\s*['\"]" + base_skill_pattern + r"['\"][\s\S]*?[\"']modifiers[\"']:\s*\[)([\s\S]*?)(\])"
...
print("Bone Spirit mapped successfully.")
