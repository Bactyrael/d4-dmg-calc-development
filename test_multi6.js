    ...
    for (let key in stats) {
        ...
        if (lowerKey.includes('[x]') || stat.isMultiplicative) {
            ...
            if (!isSkillSpecific && lowerKey.includes('damage') && !lowerKey.includes('critical') && !lowerKey.includes('over time') && !lowerKey.includes('dot') && !lowerKey.includes('to') && !lowerKey.includes('shadow') && !lowerKey.includes('darkness') && !lowerKey.includes('bone') && !lowerKey.includes('blood') && !lowerKey.includes('core') && !lowerKey.includes('macabre') && !lowerKey.includes('vulnerable') && !lowerKey.includes('cold') && !lowerKey.includes('poison') && !lowerKey.includes('lightning') && !lowerKey.includes('physical') && !lowerKey.includes('wither') && !lowerKey.includes('frailty') && !lowerKey.includes('hulking monstrosity')) {
                ...
                applies = true; console.log("Line " + 7674);;
            }
            
            if (lowerKey.includes('vulnerable') && conds.vulnerable) applies = true; console.log("Line " + 7677);;
            
            if (lowerKey.includes('damage to') && (lowerKey.includes('shadow damage over time') || lowerKey.includes('affected by shadow'))) {
                if (conds.shadowDot) applies = true; console.log("Line " + 7680);;
            }
            ...
            if (isShadowStat && !isDotStat && !lowerKey.includes('damage to')) {
                if (tags.includes('skill_shadow') || tags.includes('search_shadow') || tags.includes('skill_darkness') || dType === 'shadow') applies = true; console.log("Line " + 7687);;
            }
            if (lowerKey.includes('bone') && (tags.includes('skill_bone') || tags.includes('search_bone') || dType === 'bone')) applies = true; console.log("Line " + 7689);;
            if (lowerKey.includes('blood') && tags.includes('skill_blood')) applies = true; console.log("Line " + 7690);;
            if (lowerKey.includes('core') && tags.includes('keyword_core')) applies = true; console.log("Line " + 7691);;
            if (lowerKey.includes('macabre') && (tags.includes('keyword_macabre') || tags.some(t => t.toLowerCase().includes('macabre')) || ['Bone Prison', 'Blood Mist', 'Golem', 'Bone Spirit'].includes(skill.name))) applies = true; console.log("Line " + 7692);;
            if (lowerKey.includes('cold') && (tags.includes('skill_cold') || tags.includes('search_cold') || dType === 'cold')) applies = true; console.log("Line " + 7693);;
            if (lowerKey.includes('poison') && (tags.includes('skill_poison') || tags.includes('search_poison') || dType === 'poison')) applies = true; console.log("Line " + 7694);;
            if (lowerKey.includes('lightning') && (tags.includes('skill_lightning') || tags.includes('search_lightning') || dType === 'lightning')) applies = true; console.log("Line " + 7695);;
            if (lowerKey.includes('physical') && (tags.includes('skill_physical') || tags.includes('search_physical') || dType === 'physical')) applies = true; console.log("Line " + 7696);;
            
            if (lowerKey === 'wither damage [x]') {
                if (tags.includes('skill_darkness') || tags.includes('search_darkness') || tags.includes('skill_shadow') || tags.includes('search_shadow') || dType === 'shadow') applies = true; console.log("Line " + 7699);;
                if (tags.includes('skill_cold') || tags.includes('search_cold') || dType === 'cold') applies = true; console.log("Line " + 7700);;
            }
            if (lowerKey === 'blood begets blood damage [x]') applies = true; console.log("Line " + 7702);;
            if (lowerKey === 'frailty damage [x]' && conds.cursed) applies = true; console.log("Line " + 7703);;
            if (lowerKey.includes('cult leader')) {
                let sName = skill.name.toLowerCase();
                if (sName.includes('golem') || sName.includes(' mage') || sName.includes('warrior') || tags.some(t => t.includes('minion'))) applies = true; console.log("Line " + 7706);;
            }
            if (lowerKey === 'hulking monstrosity damage [x]') {
                if (skill.name.toLowerCase().includes('golem')) applies = true; console.log("Line " + 7709);;
            }
            if (lowerKey === 'reaper sacrifice damage [x]') {
                applies = true; console.log("Line " + 7712);;
            }
            if (lowerKey === 'cold mage sacrifice damage [x]' && conds.vulnerable) applies = true; console.log("Line " + 7714);;
            if (lowerKey === 'bone mage sacrifice damage [x]' && buffs.overpower > 0) applies = true; console.log("Line " + 7715);;
            if (lowerKey === 'bloodbath (legendary node)' && buffs.overpower > 0) applies = true; console.log("Line " + 7716);;
            if (lowerKey === 'iron golem sacrifice damage [x]' && conds.critical) applies = true; console.log("Line " + 7717);;
            
            if (lowerKey.includes('deadraiser') || lowerKey.includes('commander')) {
                let sName = skill.name.toLowerCase();
                if (sName.includes('golem') || sName.includes(' mage') || sName.includes('warrior') || tags.some(t => t.includes('minion'))) applies = true; console.log("Line " + 7721);;
            }
            if (lowerKey.includes('golem (legendary bonus)') || lowerKey.includes('golem (additional bonus)')) {
                if (skill.name.toLowerCase().includes('golem')) applies = true; console.log("Line " + 7724);;
            }
            if (lowerKey.includes('amplify')) {
                if (conds.cursed) applies = true; console.log("Line " + 7727);;
            }
            if (lowerKey.includes('control (legendary bonus)')) {
                if (conds.cc) applies = true; console.log("Line " + 7730);;
            }
            if (lowerKey.includes('scent of death') && conds.corpsesNearby) applies = true; console.log("Line " + 7732);;
            if (lowerKey.includes('territorial') && conds.close) applies = true; console.log("Line " + 7733);;
            ...
            if (!lowerKey.includes('cult leader') && !lowerKey.includes('deadraiser') && !lowerKey.includes('commander') && !lowerKey.includes('golem') && !lowerKey.includes('amplify') && !lowerKey.includes('control') && !lowerKey.includes('scent of death') && !lowerKey.includes('territorial') && !lowerKey.includes('damage') && !lowerKey.includes('critical') && !isDotStat && !isShadowStat && !lowerKey.includes('bone') && !lowerKey.includes('blood') && !lowerKey.includes('core') && !lowerKey.includes('macabre') && !lowerKey.includes('vulnerable') && !lowerKey.includes('cold') && !lowerKey.includes('poison') && !lowerKey.includes('lightning') && !lowerKey.includes('physical')) {
                applies = true; console.log("Line " + 7737);;
            }
            ...
            if (isDotStat && !lowerKey.includes('damage to')) {
                if (tags.includes('search_dot') || tags.includes('search_shadowdot')) {
                    applies = true; console.log("Line " + 7743);;
                    if (isShadowStat) {
